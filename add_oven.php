...
if(isset($_POST['add_oven'])){
    ...
    if(empty($oven_name) || empty($oven_price) || empty($oven_image) || empty($oven_quantity)){
        ...
        $insert = mysqli_query($conn,"INSERT INTO oven(name, price, image,quantity) VALUES('$oven_name', '$oven_price', '$oven_image' , '$oven_quantity')");
...
if(isset($_GET['delete'])){
    $id = $_GET['delete'];
    mysqli_query($conn, "DELETE FROM oven WHERE id = $id");
...
if(isset($_POST['add_to_cart'])){
    ...
    $oven_name = $_POST['oven_name'];
    $oven_price = $_POST['oven_price'];
    $oven_image = $_POST['oven_image'];

    $oven_quantity = $_POST['oven_quantity '];

    $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE name = '$oven_name'AND user_id = '$user_id'") or die('query failed');

    if(mysqli_num_rows($select_cart) > 0){
        $message[] = 'product already added to cart';
    }else{
        $insert_product = mysqli_query($conn, "INSERT INTO `cart`(user_id,name, price, image, quantity) VALUES('$user_id','$oven_name', '$oven_price', '$oven_image', '$oven_quantity')");
...
<div class="container">
    ...
    <div class="admin-product-form-container" style="margin-top:100px ">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <input type="text" placeholder="enter oven name" name="oven_name" class="box">
            <input type="number" placeholder="enter oven price" name="oven_price" class="box">
            <input type="number" placeholder="enter oven quantity" name="oven_quantity" class="box">
            <input type="file" accept="image/png, image/jpeg, image/jpg" name="oven_image" class="box">
            <input type="submit" class="ww" name="add_oven" value="add oven">
        </form>
    ...
    $select = mysqli_query($conn, "SELECT * FROM oven");
        ...
        <table class="product-display-table">
            ...
            <?php while($row = mysqli_fetch_assoc($select)){ ?>
                <tr>
                    ...
                    <td>
                        <a class="ww" href="update_oven.php?edit=<?php echo $row['id']; ?>" > <i class="fas fa-edit"></i> edit </a>
                        <a class="ww" href="add_oven.php?delete=<?php echo $row['id']; ?>"> <i class="fas fa-trash"></i> delete </a>
                    </td>
                </tr>
            <?php } ?>
        </table>
